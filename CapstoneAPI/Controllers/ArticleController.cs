...
namespace CapstoneAPI.Controllers
{
    ...
    public class ArticleController : ControllerBase
    {
        ...
        [HttpPut()]
        public async Task<ActionResult<Response<ApprovingArticleResponse>>> ApprovingArticle([FromBody] ApprovingArticleDataSet approvingArticleDataSet)
        {
            string token = Request.Headers["Authorization"];
            Response<ApprovingArticleResponse> result = await _service.ApprovingArticle(approvingArticleDataSet, token);
            if (result == null)
                return NoContent();
            return Ok(result);
        }
